...
// Client implements E-Katalog feed generator
// E-Katalog is a popular price comparison site in Ukraine
type Client struct {
	config *marketplace.Config
}

// New creates a new E-Katalog client
func New() *Client {
	return &Client{}
}
...
// ExportProducts - E-Katalog fetches feed from URL
...
// ImportOrders - E-Katalog redirects to shop
...
// GetCategories returns E-Katalog categories
func (c *Client) GetCategories(ctx context.Context) ([]marketplace.Category, error) {
	return nil, nil
}

// GenerateFeed generates E-Katalog YML feed
func (c *Client) GenerateFeed(ctx context.Context, products []*marketplace.Product) ([]byte, error) {
	return c.GenerateEKatalogFeed(products)
}

// GenerateEKatalogFeed generates E-Katalog specific YML feed
func (c *Client) GenerateEKatalogFeed(products []*marketplace.Product) ([]byte, error) {
	...
	for _, p := range products {
		...
		if p.Warranty > 0 {
			buf.WriteString(fmt.Sprintf("      <manufacturer_warranty>true</manufacturer_warranty>\n"))
			buf.WriteString(fmt.Sprintf("      <param name=\"Гарантія\">%d міс.</param>\n", p.Warranty))
		}
